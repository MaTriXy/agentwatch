...
def mock_setup():
    ...
    with patch('multiprocessing.Process') as mock_process, \
        ...
        mock_spyllm_fd = mock_pipe.return_value[1]
...
def client(mock_setup):
    """Fixture that creates a SpyLLMClient with mocked dependencies"""
    return SpyLLMClient()
...
def test_send_command(client):
    ...
    with patch('spyllm.pipes.Pipes.write_payload_sync') as mock_write:
...
def test_send_command_wait(client):
    ...
    with patch('spyllm.pipes.Pipes.write_payload_sync') as mock_write, \
         patch('spyllm.pipes.Pipes.read_response') as mock_read, \
         patch('spyllm.models.Command.from_dict') as mock_command:
...
def test_send_command_wait_timeout(client):
    ...
    with patch('spyllm.pipes.Pipes.write_payload_sync'), \
         patch('spyllm.pipes.Pipes.read_response') as mock_read:
